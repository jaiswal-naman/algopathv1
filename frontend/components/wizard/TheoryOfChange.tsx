...
export function TheoryOfChange({ lfaDocument }: TheoryOfChangeProps) {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-slate-800/50 border border-slate-700 rounded-xl p-6">
        <div>
      ...
      <div className="relative h-3 bg-slate-800 border border-slate-700 rounded-full overflow-hidden">
        <div
          className="absolute inset-y-0 left-0 bg-gradient-to-r from-amber-500 via-blue-500 via-purple-500 to-emerald-500 transition-all duration-500"
          style={{ width: `${(currentStep / (totalSteps - 1)) * 100}%` }}
        />
        {showParticles && isPlaying && (
          <div
            className="absolute h-5 w-5 -top-1 rounded-full bg-white shadow-lg shadow-emerald-500/50 animate-pulse"
        ...
        <CardContent className="pt-6 overflow-x-auto">
          <div className="flex items-stretch gap-4 min-w-[1200px]">
            ...
            <div className="flex-1 space-y-3">
              <div
                className={`flex items-center gap-2 p-2 rounded-lg ${getActiveState("activities", 0)
                  ? `${LEVEL_COLORS.activities.bg} ${LEVEL_COLORS.activities.border} border-2`
                  : "bg-muted/50"
                  }`}
              >
                <Activity
                  className={`h-5 w-5 ${getActiveState("activities", 0)
            ...
            <div className="flex items-center justify-center w-12">
              <div
                className={`flex flex-col items-center gap-1 transition-all duration-500 ${getConnectorState("activities", "outputs")
                  ? "opacity-100"
                  : "opacity-30"
                  }`}
              >
                <div
                  className={`w-8 h-1 rounded-full ${getConnectorState("activities", "outputs")
                    ? "bg-gradient-to-r from-amber-500 to-blue-500"
                    : "bg-muted"
                    }`}
                />
                <ArrowRight
                  className={`h-6 w-6 ${getConnectorState("activities", "outputs")
            ...
            <div className="flex-1 space-y-3">
              <div
                className={`flex items-center gap-2 p-2 rounded-lg ${getConnectorState("activities", "outputs")
                  ? `${LEVEL_COLORS.outputs.bg} ${LEVEL_COLORS.outputs.border} border-2`
                  : "bg-muted/50"
                  }`}
              >
                <Package
                  className={`h-5 w-5 ${getConnectorState("activities", "outputs")
            ...
            <div className="flex items-center justify-center w-12">
              <div
                className={`flex flex-col items-center gap-1 transition-all duration-500 ${getConnectorState("outputs", "outcomes")
                  ? "opacity-100"
                  : "opacity-30"
                  }`}
              >
                <div
                  className={`w-8 h-1 rounded-full ${getConnectorState("outputs", "outcomes")
                    ? "bg-gradient-to-r from-blue-500 to-purple-500"
                    : "bg-muted"
                    }`}
                />
                <ArrowRight
                  className={`h-6 w-6 ${getConnectorState("outputs", "outcomes")
            ...
            <div className="flex-1 space-y-3">
              <div
                className={`flex items-center gap-2 p-2 rounded-lg ${getConnectorState("outputs", "outcomes")
                  ? `${LEVEL_COLORS.outcomes.bg} ${LEVEL_COLORS.outcomes.border} border-2`
                  : "bg-muted/50"
                  }`}
              >
                <TrendingUp
                  className={`h-5 w-5 ${getConnectorState("outputs", "outcomes")
            ...
            <div className="flex items-center justify-center w-12">
              <div
                className={`flex flex-col items-center gap-1 transition-all duration-500 ${getConnectorState("outcomes", "goal")
                  ? "opacity-100"
                  : "opacity-30"
                  }`}
              >
                <div
                  className={`w-8 h-1 rounded-full ${getConnectorState("outcomes", "goal")
                    ? "bg-gradient-to-r from-purple-500 to-green-500"
                    : "bg-muted"
                    }`}
                />
                <ArrowRight
                  className={`h-6 w-6 ${getConnectorState("outcomes", "goal")
            ...
            <div className="flex-1 space-y-3">
              <div
                className={`flex items-center gap-2 p-2 rounded-lg ${getConnectorState("outcomes", "goal")
                  ? `${LEVEL_COLORS.goal.bg} ${LEVEL_COLORS.goal.border} border-2`
                  : "bg-muted/50"
                  }`}
              >
                <Target
                  className={`h-5 w-5 ${getConnectorState("outcomes", "goal")
